            ...
            dataQC.map(variant => {
                ...
                variant['exclusions'].map((e, index) => { 
                    ...
                    const axiom_batches = e.axiom_batches.length === 0 ? null : 
                        e.axiom_batches.length === variant.total_axiom_batch_count ? 'All Axiom' : e.axiom_batches.map(element => element['BATCH'].split('_')[1]).join(',');
                                            
                    const leagacy_batches = e.legacy_batches.length === 0 ? null : 
                        e.legacy_batches.length === variant.total_legacy_batch_count ? 'All legacy' : e.legacy_batches.map(element => element['BATCH']).join(',');
